...
public:
    ...
    ll update(int node, int start, int end, int t_start, int t_end, ll t_value){
        if(t_start <= start && end <= t_end) {
            lazy[node] ^= t_value;
            int check = (end - start + 1) % 2;
            return check * t_value;
        }
        if(t_end < start || t_start > end) return 0;
        
        int mid = (start + end) / 2;
        ll update_value = update(node*2, start, mid, t_start, t_end, t_value) ^ update(node*2+1, mid+1, end, t_start, t_end, t_value);
...
int main()
{
    ...
    for(int i=0; i<M; i++){
        ...
        if(a == 1){
            ...
            cin >> b >> c;
            cout << seg_tree->query(1, 1, N, b+1, c+1) << '\n';
        }
    }
    delete seg_tree;
}